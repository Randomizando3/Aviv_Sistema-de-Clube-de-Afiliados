<?php
// file: api/controllers/FormController.php  (ajuste o caminho conforme seu projeto)
...
class FormController
{
    ...
    /**
     * Destino específico para formulários:
     * - lê config/mail.php['form_to_email']
     * - fallback: contato@avivmais.com.br
     */
    private function formToEmail(): string
    {
        $fallback = 'contato@avivmais.com.br';

        try {
            $cfgPath = __DIR__ . '/../../config/mail.php';
            if (!is_file($cfgPath)) return $fallback;

            $cfg = require $cfgPath;
            $to = trim((string)($cfg['form_to_email'] ?? ''));
    ...
    public function partner(): void
    {
        try {
            ...
            $send = $mailer->send([
                ...
                'to'       => $this->formToEmail(), // AGORA: vem do EMAIL_FORM_TORECEIVE
                'html'     => $html,
